package org.hilingual.domain.diaryfeedback.api.service.diff.builder;
...
public class DiffRangeBuilder {
    ...
    public List<DiaryDetails.DiffRange> buildDiffRanges(List<DiffOperation> operations, String rewriteText) {
        ...
        for (DiffOperation operation : operations) {
            switch (operation.getType()) {
                case INSERT -> {
                    ...
                    diffRanges.add(new DiaryDetails.DiffRange(start, end, insertWord.getOriginalWord()));
                    rewriteIndex = end;
                }
                case EQUAL -> {
                    // 동일한 단어의 위치 업데이트
